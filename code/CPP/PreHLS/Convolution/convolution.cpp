            ...
            for (int kerW=-CNN_KERNEL_W/2; kerW<=CNN_KERNEL_W/2;kerW++)
            {
              for(int inChannel=0;inChannel<canal_in;inChannel++)
              {
                if(row+kerH>=0 && row+kerH<height_in && col+kerW>=0 && col+kerW<width_in){
                  s=s+(CNN_DATA_OUT_TYPE)datain[width_in*canal_in*(row+kerH)+canal_in*(col+kerW)+inChannel]*weights[CNN_KERNEL_W*canal_in*canal_out*(kerH+1)+canal_in*canal_out*(kerW+1)+canal_out*inChannel+outChannel];
                }
              }
            }
          }
